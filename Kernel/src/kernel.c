...
int main(){
	...
	socketconsola = inicializar_servidor(puertoEscucha, ipEscucha);
...
void conexionMemoria(t_config* arg1){
	...
	char puertoMemoria = config_get_string_value(arg1, "PUERTO_MEMORIA");

	int conexionMemoria;

	conexionMemoria = inicializar_cliente(ipMemoria,puertoMemoria);

	log_info(logger, "Inicializado cliente de conexion con Memoria, socket: %d.", conexionMemoria);
